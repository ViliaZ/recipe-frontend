...
export async function createRecipe(recipe) {
  let data;
  try {
    data = JSON.parse(fs.readFileSync("data/recipes.json")); // Pfad zu Ihrer JSON-Datei
  } catch (error) {
    data = { recipes: [] };
  }
  ...
  fs.writeFileSync("data/recipes.json", JSON.stringify(data, null, 2)); // Pfad zu Ihrer JSON-Datei
